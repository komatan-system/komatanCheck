...
function getSeat() {
  ...
  const resApi = callSeatApi();
  if (!resApi) return;  

  const dayWeekFromSS = getDayWeek();
  Logger.log(dayWeekFromSS);  

  const subjectFromSS = getSubject();
  ...
  for (const entry of parsedJson.data) {
    ...
    const d = new Date(entry.date + 'T00:00:00+09:00'); // JSTの0時として作る
    const names = ['日','月','火','水','木','金','土'];
    const dayOfWeek = names[d.getDay()];
    const date = `${entry.date}(${dayOfWeek})`;
    Logger.log(date);
